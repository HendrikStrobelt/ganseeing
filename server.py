...
import argparse, connexion, os, yaml, json, socket
import torch
...
# from gand.easydict import EasyDict
# from gand.serverstate import GANPaintProject
...
def post_generate(gen_req):
    ...
    ids = gen_req.get('ids')
    p = projects[project]  # type: InverterProject
    # p.generate_images(ids)
    return {
        'request': gen_req,
        'res': ''
    }
...
def post_upload(upload_req):
    ...
    res = p.invert_generate(image_data)
    # print(res)

    # p_i = imagedata.pil_from_tensor(res['input_t'])
    # p_o = imagedata.pil_from_tensor(res['output_t'])

    # p_diff = imagedata.pil_from_tensor(
    #     torch.abs(res['input_t'] - res['output_t']))
...
# def post_upload_all(upload_req):
#     image_data = upload_req['image']
#     # print(image_data)
...
#         p = projects[project_key]  # type: InverterProject
#         res = p.invert_generate(image_data)
#         p_o = imagedata.pil_from_tensor(res['output_t'])
#         p_diff = imagedata.pil_from_tensor(
#             torch.abs(res['input_t'] - res['output_t']))
#         ops[project_key] = {'p_o': pil_to_base64(p_o), 'p_diff': pil_to_base64(p_diff)}

#     p_i = imagedata.pil_from_tensor(res['input_t'])

#     return {
#         'res': {
#             'input': pil_to_base64(p_i),
#             'outputs': ops
#         }
#     }
...
@app.route('/')
def redirect_home():
    return redirect('/client/gandiff.html', code=302)
...
parser.add_argument("--client", type=str, default='client/dist')
